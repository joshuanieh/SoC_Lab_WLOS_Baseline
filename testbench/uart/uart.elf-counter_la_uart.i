# 0 "counter_la_uart.c"
# 1 "/mnt/c/Users/nieh/Documents/Joshuanieh/GIEE/Soc_Design_Lab/SoC_Lab_WLOS_Baseline/testbench/uart//"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "counter_la_uart.c"
...
extern int* matmul();
extern int* fir();
# 48 "counter_la_uart.c"
void main()
{

    int mask;
# 79 "counter_la_uart.c"
 ...
 int j;
 for(j=0; j<16; j++){
  (*(volatile uint32_t*)0x2600000c) = *(tmp1+j) << 16;
 }
 ...
 int* tmp2 = qsort();
 for(j=0; j<10; j++){
  (*(volatile uint32_t*)0x2600000c) = *(tmp2+j) << 16;
 }
# 172 "counter_la_uart.c"
